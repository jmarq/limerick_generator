...
def same_number_of_syllables(sentence_list):
    ...
    syllable_count_list = map(lambda(s): sentence_syllables(s), sentence_list)
    return equal_list(syllable_count_list)


def phones_for_phrase(phrase):
    words = phrase.lower().strip().split()
    phones_for_words = [pronouncing.phones_for_word(strip_punc(word))[0] for word in words]
...
def vowel_rhyme(phrase_list, num_syllables=2):
    ending_vowel_phones = map(lambda(p): vowel_phones_for_last_n_syllables(p, num_syllables), phrase_list)
